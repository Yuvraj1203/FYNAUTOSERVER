...
from fynautoserver.utils.index import create_response, extract_fonts_from_ts
...
async def create_fonts(
    ...
    if existing:
        if defaultFont:
            ...
            destination_folder_Assets =  f"src/tenant/tenants/{tenancyName}/assets/fonts"
            shutil.copytree(source_folder_Assets, destination_folder_Assets,dirs_exist_ok=True)

            #get the name from index file and send it to db
            fonts = extract_fonts_from_ts(os.path.join(SRC_DIR, f"tenant/tenants/{tenancyName}/assets/fonts/index.tsx"))
            print(fonts,'==============================this is from fomnts saved=================>>>>>>>>>>')
            
            # Update DB record with new font path and name
            destination_folder_for_db =  f"tenant/tenants/{tenancyName}/assets/fonts/AppFont"
            existing.lightFontPath = os.path.join(destination_folder_for_db, fonts['Light'])
            existing.regularFontPath = os.path.join(destination_folder_for_db, fonts['Regular'])
